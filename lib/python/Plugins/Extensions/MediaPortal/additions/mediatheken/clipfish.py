...
class show_CF_Genre(MenuHelper):
	...
	def mh_parseHighlightsData(self, data):
		entrys = []
		for ms in re.finditer('<div class="cf-box-top">\s+<h1>(.+?highlights)</h1>.*?(.*?)</ul>', data, re.S|re.I):
...
class CF_FilmListeScreen(MPScreen, ThumbsHelper):
	...
	def __init__(self, session, genreLink, genreName, genreImg=None):
		...
		self.genreShows = '/special/comedy/' in self.genreLink and not '/special/comedy/videos' in self.genreLink
