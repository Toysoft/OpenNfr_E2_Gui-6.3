...
default_cover = "file://%s/bitporno.png" % (config.mediaportal.iconcachepath.value + "logos")
...
class bitpornoFilmScreen(MPScreen, ThumbsHelper):
	...
	def loadData(self, data):
		...
		Movies = re.findall('class="entry.*?href="(.*?)".*?img\ssrc="(.*?)".*?(<div.*?)</div', data, re.S)
		if Movies:
			for (Url, Image, Title) in Movies:
				...
				blacklist = [	'dagashi-kashi', 'mahoutsukai-no-yome', 'ryuuou-no-oshigoto', 'b-gata-h-kei', 'misa-anime', 'sword-art-online',
						...
						'imouto-sae-ireba', 'himouto-umaru', 'hand-shakers', 'little-witch-academia', 'kujira-no-kora']
				for x in blacklist:
					if x in Title.lower():
						stop = True
