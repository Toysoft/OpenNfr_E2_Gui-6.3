...
class topPornGenreScreen(MPScreen):
	...
	def genreData(self, data):
			...
			self.genreliste.insert(0, ("Most Viewed", "http://top1porn.com/top-viewed", None))
			self.genreliste.insert(0, ("Newest", "http://top1porn.com/new-movies", None))
...
class topPornFilmScreen(MPScreen, ThumbsHelper):
	...
	def loadData(self, data):
		self.getLastPage(data, '', 'class="inactive\sarrow\slast"\shref="http://top1porn.com/.*?/page/(.*?)"><span\sclass="theme-icon-last">')
		parse = re.search('-content loop">(.*?)class="pagination-wrapper">', data, re.S)
		Movies = re.findall('class="top-ten-link.*?href="(.*?)".*?title="(.*?)"><img\ssrc="(.*?)"\salt="', parse.group(1), re.S)
		if Movies:
