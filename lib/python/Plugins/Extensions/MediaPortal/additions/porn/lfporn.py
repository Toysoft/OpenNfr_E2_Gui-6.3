...
default_cover = "file://%s/lfporn.png" % (config_mp.mediaportal.iconcachepath.value + "logos")
...
class LFPornGenreScreen(MPScreen):
	...
	def layoutFinished(self):
		self.keyLocked = True
		url = "http://www.lfporn.com/"
		getPage(url, agent=myagent).addCallback(self.genreData).addErrback(self.dataError)

	def genreData(self, data):
		parse = re.search('<h3>Categories</h3>(.*?)</ul>', data, re.S)
		Cats = re.findall('<a\shref="(.*?)"\stitle="(.*?)">', parse.group(1), re.S)
		if Cats:
	...
	def SuchenCallback(self, callback = None):
		if callback is not None and len(callback):
			...
			self.session.open(LFPornFilmScreen, Link, Name)
...
class LFPornFilmScreen(MPScreen, ThumbsHelper):
	...
	def loadPage(self):
		...
		if re.match(".*?Search", self.Name):
			url = "http://www.lfporn.com/page/" + str(self.page) + "/?s=" + self.Link
		else:
	...
	def loadData(self, data):
		if re.match(".*?Search", self.Name):
			...
			self.getLastPage(data, 'class="paginator(.*?)</div>', '.*[>|=|\/](\d+)[<|"|\/]')
		Movies = re.findall('<div\sclass="post".*?<a\shref="(.*?)".*?title="(.*?)"><img.*?src="(.*?)"\sclass="', data, re.S)
		if Movies:
	...
	def keyOK(self):
		...
		if url:
			self.session.open(LFPornFilmAuswahlScreen, title, url, image)

class LFPornFilmAuswahlScreen(MPScreen):
	...
	def got_link(self, stream_url):
		...
		self.session.open(SimplePlayer, [(title, stream_url)], showPlaylist=False, ltype='lfporn')