...
class FastScanScreen(ConfigListScreen, Screen):
	...
	def __init__(self, session, nimList):
		...
		self.providers = {}
        	self.providers['Sky_Orf_De'] = (0, 900, True)
        	self.providers['Sky_HD'] = (0, 900, True)
        	self.providers['Sky_SD'] = (0, 900, True)
	...
	if session.nav.RecordTimer.isRecording():
		...
		for n in nimmanager.nim_slots:
			...
			if n.config.dvbs.configMode.value == "nothing":
				continue
			if n.isCompatible("DVB-S") and n.config.dvbs.configMode.value in ("loopthrough", "satposdepends"):
