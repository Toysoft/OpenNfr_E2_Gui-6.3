...
class UpdatePluginMenu(Screen):
	...
	def __init__(self, session, args = 0):
		...
		if self.menu == 0:
			...
			self.list.append(("channel-backup", _("Backup current Channellist"), _("\nBackup your %s %s Channellist.") % (getMachineBrand(), getMachineName()) + self.oktext + "\n\n" + self.infotext, None))
	...
	def go(self, num = None):
		...
		if current:
			...
			if self.menu == 0:
				...
				elif currentEntry == "channel-backup":  
					self.session.openWithCallback(self.backupDone,Dream_BackupScreen, runBackup = True)
...
def Plugins(path, **kwargs):
	...
	if not config.plugins.softwaremanager.onSetupMenu.value and not config.plugins.softwaremanager.onBlueButton.value:
		list.append(PluginDescriptor(name=_("Software management"), description=_("Manage your %s %s's software") % (getMachineBrand(), getMachineName()), icon='SoftwareManager.png' , where = PluginDescriptor.WHERE_PLUGINMENU, needsRestart = False, fnc=UpgradeMain))
	if config.plugins.softwaremanager.onBlueButton.value:
		list.append(PluginDescriptor(name=_("Software management"), description=_("Manage your %s %s's software") % (getMachineBrand(), getMachineName()), icon='SoftwareManagerFHD.png' , where = PluginDescriptor.WHERE_EXTENSIONSMENU, needsRestart = False, fnc=UpgradeMain))
	return list 
