...
from boxbranding import getBoxType, getBrandOEM, getMachineProcModel, getMachineBuild, getMachineBrand, getMachineBrand, getMachineName
...
#			print "seekable"
			...
			self.activityTimer.start(int(config.seek.withjumps_repeat_ms.value), False)
...
#			print "not pauseable."
	...
	def doSeekRelativeAvoidStall(self, pts):
		...
		if seekable and config.seek.withjumps_avoid_zero.value:
	...
	def DoSeekAction(self):
		if self.seekAction > int(config.seek.withjumps_after_ff_speed.value):
			self.doSeekRelativeAvoidStall(self.seekAction * long(config.seek.withjumps_forwards_ms.value) * 90)
		elif self.seekAction < 0:
			self.doSeekRelativeAvoidStall(self.seekAction * long(config.seek.withjumps_backwards_ms.value) * 90)

		for c in self.onPlayStateChanged:
			if self.seekAction > int(config.seek.withjumps_after_ff_speed.value): # Forward
	...
	def seekFwd_new(self):
		...
		if (self.seekAction > 1) and (self.seekAction <= int(config.seek.withjumps_after_ff_speed.value)): # use fastforward for the configured speeds
			self.setSeekState(self.makeStateForward(self.seekAction))
		elif self.seekAction > int(config.seek.withjumps_after_ff_speed.value): # we first need to go the play state, to stop fastforward
			self.setSeekState(self.SEEK_STATE_PLAY)
	...
	def seekBack_new(self):
		...
		if (self.seekAction > 1) and (self.seekAction <= int(config.seek.withjumps_after_ff_speed.value)): # use fastforward for the configured forwards speeds
			self.setSeekState(self.makeStateForward(self.seekAction))
	...
	def showPiP(self):
			...
			try:
				...
				if self.session.pipshown:
					...
					if int(xres) <= 720 or about.getCPUString() == 'BCM7346B2' or about.getCPUString() == 'BCM7425B2' or about.getCPUString() == 'BCM7429B0' or getBoxType() in ('vusolo4k', 'mutant51', 'ax51'):
